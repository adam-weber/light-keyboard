...
[`.github/workflows/release.yml`](.github/workflows/release.yml). The typing data in
`app/src/main/res/raw/` — `charmodel.bin` (tap accuracy), `lexicon.bin` and `bigram.bin` (swipe typing) —
is regenerated by the scripts in [`tools/`](tools): `gen_charmodel.py`, `gen_lexicon.py`, `gen_bigram.py`.
